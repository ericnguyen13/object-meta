...
function _changeDisplayTypeName(displayTypeName){
    ...
    this.displayTypeName = _isDisplayTypeNameValid(displayTypeName) ? displayTypeName : DisplayTypes.Text;
...
function _isDisplayTypeNameValid(displayTypeName){
    for(var type in DisplayTypes){
        if (displayTypeName === type){
            return true;
        }
    }
...
function NodeType(options){
    this.displayTypeName = options && _isDisplayTypeNameValid(options.displayTypeName) ? options.displayTypeName : DisplayTypes.Text;
    ...
    if (options && options.options){
        ...
        this.changeDisplayTypeName(this.displayTypeName);
    }
}
...
NodeType.prototype.clone = function(){
    return new NodeType({
        typeName: _isTypeNameValid(this.typeName) ? this.typeName : Types.String,
        displayTypeName: _isDisplayTypeNameValid(this.displayTypeName) ? this.displayTypeName : DisplayTypes.Text,
